...
public class AdDealsBannerAdViewModel extends ViewModelBase {
    ...
    /*internal*/ int CounterWithoutAd = 0;
    /*internal*/ int ServerRefreshRate = BANNER_REFRESH_RATE_DEFAULT;

    public AdDealsBannerAdViewModel()
    {
        this._webViewVisibility = View.INVISIBLE;
    }

    internal void RefreshScreenSize(AdManager.BannerAdSizes adType)
    {
        switch (adType)
        {
    ...
    /// </summary>
    /// <returns></returns>
    internal async Future<Integer> getNewAd(boolean strictSize)
    {
        String requestedAdTypes = BANNER_SUPPORTED_ADS; // All by default.

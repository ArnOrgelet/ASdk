...
public abstract class AbstractAdManager {
     static int DEFAULT_CAMPAIGN_ID = 2;

    static String VIDEO_COMPLETED_HTML_TAG = "videocompleted.addealsnetwork.com";
    static String VIDEO_LOADING_FAILED_HTML_TAG = "videoloadingfailed.addealsnetwork.com";
    static String VIDEO_LINKCLICKED_HTML_TAG = "videolinkclicked.addealsnetwork.com";
    static String POPUP_CLOSED_HTML_TAG = "><adclosed";
    static String VIDEO_PLAYER_RELEASE_MODE = "web.addealsnetwork.com";
    static String VIDEO_PLAYER_DEBUG_MODE = "addeals-staging.azurewebsites.net";
    ...
    static String MOBILE_OPERATOR = StringHelper.Empty;
    protected static String APP_UID = StringHelper.Empty;
    static String APP_CONNECTION = StringHelper.Empty;
    static String ADVERTISER_UID = StringHelper.Empty;
    static String USER_AGENT = StringHelper.Empty;

    // Post install notification sent parameters:
    static long ADDEALS_USER_ID = -1;
    static long ADDEALS_DOWNLOAD_ID = -1;
    static long ADDEALS_ORIGIN_CLICK_ID = -1;

    // Campaign Types (for v2 web services)
    final int CAMPAIGN_TYPE_WALL = 5;
    final int CAMPAIGN_TYPE_INTERSTITIAL = 3;
    final int CAMPAIGN_TYPE_BANNER = 2;
    final int CAMPAIGN_TYPE_VIDEO_REWARDED = 12;

    // Interstitial finalants
    final int HTTP_QUERY_TIMEOUT = 10;

    /* final int AD_ACTION_NOT_INTERESTED = 3;
     final int AD_ACTION_MORE_OFFERS = 4;
     final int AD_ACTION_REMOVE_ADS = 5;
     final int AD_ACTION_CLOSE_ADS = 6;*/

    final int NO_AD_AVAILABLE = -1;       // No Ad received from AdDeals server
    final int AD_AVAILABLE = 1;           // 1 Square ad received from AdDeals server
    final int AD_AVAILABILITY_UNKNOWN = 100;
    final int ERROR_ACCESS_DENIED = 403;
    final int ERROR_SDK_NOT_INITIALIZED = 1001;
    final int ERROR_INCOMPATIBLE_AD = 1002;

    // Campaign targets
    final int TARGET_APP_DOWNLOAD = 1;
    final int TARGET_WEB_MOBILE = 2;
    final int TARGET_CLICK_TO_CALL = 3;
    final int TARGET_CLICK_TO_VIDEO = 4;

    // Ad type ID
    final int AD_TYPE_FULL_INTERSTITIAL_NATIVE = 3;
    final int AD_TYPE_FULL_INTERSTITIAL_HTML = 7;
    final int AD_TYPE_VIDEO_HTML = 9;
    final int AD_TYPE_MEDIUM_SQUARE_PICTURE = 10;
    final int AD_TYPE_SQUARE_APP_INFO = 11;
    final int AD_TYPE_MEDIUM_SQUARE_PICTURE_HTML = 12;
    final int AD_TYPE_FULL_SQUARE_PICTURE_NATIVE = 13;
    final int AD_TYPE_FULL_SQUARE_PICTURE_HTML = 14;

    // Session Types
    final int FIRST_APP_LAUNCH = 1;
    final int OTHER_APP_LAUNCH = 0;

    // Ad Kind
    // final int AD_KIND_DEFAULT = 1;
    // final int AD_KIND_FULL_SCREEN_ADS_ONLY = 3;
    // final int AD_KIND_SQUARE_ADS_ONLY = 4;

    // System Tray properties
    // final int SYSTEM_TRAY_PORTRAIT_HEIGHT = 32;
    // final int SYSTEM_TRAY_LANDSCAPE_HEIGHT = 72;

    // App current orientation
    // final int ORIENTATION_NOT_AVAILABLE = -1;
    ...
    final String NO_SUPPORTED_ADS = "";                    // Default - none. An ad format must be passed.
    final String SQUARE_SUPPORTED_ADS = "[10]";            // [10][11] - square not supported in current SDK version.
    final String FULL_SCREEN_SUPPORTED_ADS = "[3][7]";     // [3]: Full screen native ads (portrait & landscape) - pictures only / [7]: Full screen html ads.
    final String REWARDED_VIDEO_SUPPORTED_ADS = "[9]";     // [9]: HTML based video ads (portrait & landscape) - pictures only
    final String BANNER_SUPPORTED_ADS = "[2][6][9]";       // Videos supported via banners (300x250 in general)

    // Device type finalants
    final int DEVICE_WINDOWS_PHONE = 1;
    final int DEVICE_WINDOWS_TABLET = 2;
    final int DEVICE_NOT_SPECIFIED = -1;

    // Refresh rate
    final int BANNER_REFRESH_RATE_DEFAULT_AT_LAUNCH = 300;  // Until the SDK is initialized, we check it's initialized every 2 seconds.
    final int BANNER_REFRESH_RATE_DEFAULT = 60;           // Once the SDK is initialized, we try to get ads every 60seconds by default, unless it's overriden by server data.
}
